...
class PathRecord:
    order: int
    role: str  # entra | sale
    source: str  # rast | st_read | writeRaster | saveRDS | list.files | ...
...
INPUT_FUNCTIONS = [
    'rast', 'vect', 'st_read', 'read.csv', 'readRDS', 'load', 'list.files'
]

OUTPUT_FUNCTIONS = [
    'writeRaster', 'saveRDS', 'write.csv', 'save'
]
...
def extract_balanced_call(text: str, start_idx: int) -> Tuple[str, int]:
    # start_idx points to the opening parenthesis
    depth = 0
    in_str = False
    q = None
    i = start_idx
...
def collect_repeated_chunks(records: List[PathRecord]) -> Dict[str, str]:
    ...
    repeated = {str(i): chunk for i, (chunk, n) in enumerate(counts.items(), start=1) if n >= 2}
    return repeated
...
def extract_assignments(text: str) -> Dict[str, str]:
    ...
    for m in ASSIGNMENT_RE.finditer(text):
        ...
        # keep only route-like or composable route expressions
        if (
...
def extract_input_records(text: str, symbols: Dict[str, str]) -> List[PathRecord]:
    ...
    for func in ['rast', 'vect', 'st_read', 'read.csv', 'readRDS', 'load', 'list.files']:
...
def classify_bases(symbols: Dict[str, str]) -> Tuple[Dict[str, str], Dict[str, str]]:
...
def manifest_for_script(script_path: Path) -> ScriptManifest:
    ...
    manifest.input_bases, manifest.output_bases = classify_bases(symbols)

    input_base_candidates = list(manifest.input_bases.values())
    output_base_candidates = list(manifest.output_bases.values())

    enrich_base_relative(inputs, input_base_candidates)
    enrich_base_relative(outputs, output_base_candidates)

    all_records = inputs + outputs
    manifest.repeated_chunks = collect_repeated_chunks(all_records)
...
def manifest_to_dict(m: ScriptManifest) -> Dict:
    return {
        ...
        'entra': {
            k: asdict(v) for k, v in m.inputs.items()
        },
        'sale': {
            k: asdict(v) for k, v in m.outputs.items()
        },
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description='Extrae entradas, salidas, bases y componentes de rutas desde scripts R.'
    )
    parser.add_argument(
        'inputs',
        nargs='+',
        help='Archivos .R o directorios a recorrer.'
    )
    parser.add_argument(
        '-o', '--output',
        default='r_script_paths_manifest_v0_1.json',
...
def collect_r_files(input_args: List[str]) -> List[Path]:
    files: List[Path] = []
    for item in input_args:
        ...
        if p.is_file() and p.suffix.lower() == '.r':
            ...
            files.extend(sorted(x for x in p.rglob('*.R')))
            files.extend(sorted(x for x in p.rglob('*.r')))
...
def main() -> None:
    args = parse_args()
    r_files = collect_r_files(args.inputs)
    if not r_files:
        raise SystemExit('No se encontraron archivos .R en las rutas dadas.')
