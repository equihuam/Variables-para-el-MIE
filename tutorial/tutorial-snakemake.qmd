...
Use el logo del Inecol, para que funcione asegurate de poderlo desplegar en tu diseño. También puedes descargar aquí ese logo.
...
Si todo resultó bien, ahora tienes una propuesta de script 5 en proceso de desarrollo. Deberas guardarla con el nombre que elegiste en la **carpeta de nuestra estructura canónica: scripts**. Ahora puedes hacer una primera corrida de prueba desde tu **terminal SSH**, deberás estar ubicada en /srv/iie-tutor/$USER-iie.
...
Lo siguiente es resolver como quieres que se vea el mapa. Básicamente hay que resolver como consruir la simbología, las paletas de colores de los dos tipos de mapas. Lo podemos hacer con algo parecido a la siguiente interacción con la **IA**. 
...
Para una validación auxiliada por la **IA** puedes buscar averiguar si tienes un archivo bien armado (un **qgz** es una variante de contenedor zipeado). Estas líneas abajo hacen el desempacado y luego usan el commando de bpusqueda `grep`para extraer información sobre las paletas (ráster y vectorial). Puedes copiar lo que salga de eso y comoartirlo con la **IA** para que de su valoración técnica. 
...
Toma la nueva versión del script y substituye la que tienes. Ahora puedes usasr snakemake para probar el funcionamiento. la frace `--force create_projecr_standalone` hace que snakemake corra esa tarea sí o sí. Si le cambiaste de nombre no olvides poner el que usaste realmente. 
...
Si todo funciona bien, la tarea correra el script con los parámetros adecuados y te entregará un proyecto **QGis** comoleto. Ábrelo y ve que esté a ru gusto. Si hay cosas que quieras ajustar o cambiar toma nota y pídele a la **AI** que te ayude, haz peticiones específicas, e indica de la mejor manera posible como quieres el resultado. Revisión manual en **QGis** Desktop:
...
Cuando estes satisfecha con el resultado pasa al siguiente producto, que es preparar un proyecto para que actue como servidor de mapas. Esta versión no requiere tener un diseño de mapas para imprimir. Debe adecuar los metadatos de las capas para que el servidor de mapas las ubique y las haga disponibles a través de la web. Por la forma como configuré el servidor ahora, este mapa debe llamarse forsozamente `proyecto_server.qgz`.
...
Aahora tendrás una nueva versión del script. Nuevamente substituye la que ya tienes. Estas lista para una nueva corrida de prueba, pero ahora la regla que usaras es `create_project_server`:
...
Seguramente la **IA** te propondrá algo como lo que me dió a mi. Basicamente es lo necesario para abir el zip que es el proyecto **qgz** y poderlo analizar. Revisa la especificaciójn de las leyendas y los metadatos necesarios para servir loṡmapas en la Web:
...
Para revisar valores internos. Aqí le pides que saque a la pantalla las líneas de la 850 a la 900 del *XML*. Puedes pedirle ayuda a la **IA** para interpretar lo que salga.
...
3. **WCS**. **Web Coverage Service** es la opción que entrega datos en formato raster y puede incluir series de tiempo. 

En nuestra configuración, el servidor de mapas está montado en el equipo de Octavio y llegas a él mediante el puerto **8085**. En **QGiz**, busca la sección "browser", ahí encontrarás las etiquetas **WMS**, **WFS** y **WCS**. Si das click con botón derecho del mouse, te dará oportunidad de dar de alta un nuevo enlace del tipo. Entra ahí, elige un nimbre para identificar tu conexión y en `URL` anota `http://10.0.90.76:8085/cgi-bin/qgis_mapserv.fcgi`, cambia la **IP** que tenga el equipo de Octavio en este momento. Guarda y verifica el resultado. Deberías ver que la conexión se activa y te ofrece los productos que tu proyecto ha especificado.
...
Ahoa busca la sección **WCS**  y verifica que puedes recupera la capa raster `iie_mapa_raster`. Revuerda que este canal entrega el raster como cobertura numérica y no como mapa estilizado. Le puedes pedir a la **IA** algo como esto para verificar el resultado.
...
En mi ensayo, encontré varias dificultades para que el *map server* me entregara exactamente lo que yo quería. Puedes tu tambipen intentar abordar algunos de los problemas que me encontré y desde luego los tuyos propios. Así le puedes pedir ayuda a la **IA**.
...
No olvides documentar lo que hiciste. También le puedes pedir ayuda a la **IA** para generar un documento de acompañamiento. No traté aquí nada de la documentación y registro de los metadaros. **Maricruz nos orientará sobre este tema**.
...
Se produce una versión *standalone* funcionak y estable, que contiene simbología. La versión server se produce cuidando que sea funcional en el contexto de como se ha habilitado el servidor que tenemos montado y que define una ubicación virtual base: `/data`.
...
Para el vector se generó un GeoPackage con esquema estable, incluyendo los campos valor_indice e integridad_simulada, y se aplicó simbología graduada sobre la columna integridad_simulada. La retícula vectorial se mantuvo encima del raster y con transparencia para permitir lectura conjunta.
...
2. Proyecto **QGis** reproducible: generado por Py**QGis**, sin plantilla .qgz heredada.
