...
Instalé en el equipo de Octavio a **R** y **RStudio server** (en Linux no hay una aplicación de escritorio como la de Windows). Esto quiere decir que si te quieres ahorrar lo de **WinSCP**, bien podrías hacer todo desde le browser. La forma de activar ese **RStudio** es poner en el navegador la misma **IP** (@def-consigue-ip) que te funcione para el **ssh** seguido de **:8787**, contesta lo que te pregunte (tus credenciales). ¡Listo!, tendrás frente a ti todo lo necesario para operar el flujo de trabajo.

![http://10.0.122.35:8787](../img/rstudio.png){#fig-rstudio}

Me encantará saber cuál de todas las opciones que les he preparado les parece más natural y fácil de operar.



Conviene por muchas razones crear un *Proyecto de RStudio* localizado directamente dentro de la carpeta de Linux que acabas de hacer. Con este proyecto ahí, el panel Files se "anclará" automáticamente a esa ruta cada vez que abras el proyecto. Para construir este espacio en RStudio, ve a File $\rightarrow$ New Project...Selecciona Existing Directory. Haz clic en Browse... y en la barra de direcciones de la ventana que se abre, elige el la unidad RaiDrive que acabas de crear. Dale clic a Create Project y listo. A partir de ahora, RStudio abrirá el panel Files exactamente en esa ruta nativa de WSL, y todos los scripts usarán rutas relativas de manera natural. Si optas por hacerlo con **RStudio server**, harás lo mismo, pero será más simple, pues ya estas ubicada en la máquina Linux en la que está ocurriendo todo.

### Entorno de trabajo

Cuando inicias en tu sesión **SSH**, seguramente notarás la frase `qgis_env`. Se trata de una indicación de que ya estas ubicada en un **ambiente virtual de trabajo** que contiene todo lo necesario para hacer mapas con **QGis** y también está ahí habilitado **Snakemake**. Hay otros dos ambientes posibles `base` y `profiling_env`. El primero es la condición de la máquina sin ninguna preparación, probablemente no tendrás ahí ni siquiera a Python. Llegas a esa configuración con `micromamba deactivate` o con `micromamba activate base`. Hay ocasiones en las que conviene tener tal configuración, pero no será el caso en este tutorial. El otro *ambiente virtual* tiene instalada la paquetería necesaria para hacer *data profiling*. Esto quiere decir que puedes darle cualquier **CSV** con datos organizados por columnas, y te entregará un análisis muy completo de los atributos que tienen las variables y sus peculiaridades estadísticas. No lo usaremos en este tutorial, pero es algo que tiene mucho interés para las nuevas variables que estaremos procesando. Puedes hacer un análisis descriptivo de este tipo escribiendo dentro de ese entorno el comando `ydata-profiling <<ruta al archivo csv que quieras>>`. Te entregará un reporte en una archivo `HTML` que verás como una página Web. La forma de activar ese entorno virtual es `micromamba activate profiling_env`. Para los fines de este tutorial, debes asegurarte de tener activo el entorno virtual `qgis_env`. Si no ves tal indicación en tu línea de comandos de tu terminal, puedes activarlo con `micromamba activate qgis_env`en el momento en el que lo necesites.

El Ejercicio que desarrollarás en este tutorial te hará viajar a lo largo del siguiente diagrama de flujo.



```{mermaid}
%%| fig-cap: "Diagrama de Bloques del Flujo de Trabajo Geoespacial Automatizado (DAG de Snakemake)"
%%| label: fig-dag
%%| fig-width: 10
%%| fig-align: center
...
Con el botón que está aquí abajo podrás descargar los datos que se requieren para arrancar el workflow que se describe en este tutorial.

{{< downloadthis registros_base.csv dname="registros_base" label="Datos iniciales" type="warning" >}}

## 4. Configuración del `snakemake`

El archivo `config/config.yaml` se usa para especificar los parámetros del modelo sin tener que tocar una sola línea del código de los scripts. Al usar **rutas relativas**, el flujo funcionará perfectamente sin importar qué usuario lo esté corriendo. Cada vez que tengas los scripts y las reglas que los usan en este snakefile, podrás correr `snakemake --cores  1`, lo que implica poner en marcha el workflow hasta el punto de desarrollo que lleves. 

En los pasos siguientes verás versiones progresivas del mismo `snakefile`, que iras construyendo conforme agregues tareas y procesos al flujo de trabajo. Cada vez Que tengas un nuevo `snakefile`, con sus scripts y componentes necesarios, podrás correr el comando `snakemake --cores 1` para verificar que todo está en orden y si es así,  obtenedrás los productos involucrados en cada paso y que especificaste en la línea **output:**.
...
Ubicado en la raíz de tu carpeta está el archivo (`/srv/iie-tutor/$USER-iie/Snakefile`). Este archivo le indica a `Snakemake`  cómo conectar las entradas y salidas de las 4 tareas del ejercicio de capacitación descrito en este tutorial.
...
Con el archivo `snakefile` que acabas de construir, ya tienes definido el proceso de trabajo que harás enseguida. Ve lo que marcaste en las líneas que dicen **script:**, para cada regla. Eso indica que scripts deberás desarrollar para que el flujo de trabajo haga su tarea. A continuación te sugiero los tres scripts que ya están indicados en el `snakefile` que acabas de terminar. Este flujo de trabajo también incluye un reporte en **Quarto**, que es el último componente de esta primera tarea y para el cual también te ofrezco una sugerencia. Constrúyelos y dales el nombre y ubicación que están registrados en el `snakefile`.
...
Este escript está escrito en Python y crea un mapa vectorial (malla) a partir de los datos tabulares.
...
Ya tienes todos los ingredientes para hacer una primera corrida del flujo de trabajo. y si todo está bien deberás ver un montón de mensajes en pantalla y tendrás los primeros resultados en las carpetas de salida que indicaste en el mismo `snakefile` bajo el encabezado **output**.


## 7. Ejecución desde RStudio
...
## 8. ¿Cuántos núcleos tenemos disponibles?
...
## 10. Recolección tabular de las variables

La siguiente tarea es recolectar los datos y producir un arreglo centralizado a partir de las múltiples fuentes de información geoespacial, sin importar su formato de origen (ráster o vectorial). 

#### Regla en el `Snakefile`

Para asegurar que el motor de Snakemake reconozca adecuadamente el flujo, la regla utiliza un mapeo posicional explícito. La sintaxis del bloque final queda estructurada de la siguiente manera:
...
El script `scripts/6_agregar_datos.py` realiza un escaneo dinámico y recursivo en el subárbol de la carpeta `resultados/`. Muestrea los valores de píxel de los rásteres y extrae las columnas de datos de las capas vectoriales descubiertas.


```python
{{< include scripts-y-reglas/6_agregar_datos.py >}}
```

> **Nota de Diseño Importante:** Para evitar distorsiones y advertencias críticas de GeoPandas al calcular distancias en grados elipsoidales, el cálculo del centroide (`.centroid`) y el join espacial de vecindad (`sjoin_nearest`) se ejecutan estrictamente bajo un **CRS métrico (EPSG:6372)**. Posteriormente, el set de datos se reproyecta a **WGS84 (EPSG:4326)** para extraer coordenadas en formato decimal puro.

### Criterios de Validación del Entregable Tabular

Una vez completado el pipeline (`15 of 15 steps (100%) done`), el archivo consolidado estará disponible en `resultados/dataset_agregado.csv`. Puedes realizar una inspección directa mediante comandos de consola para verificar su estructura *Tidy Data*:

```bash
head -n 5 resultados/dataset_agregado.csv

```

El archivo final debe cumplir rigurosamente las siguientes expectativas de formato:

1. **Ausencia de cadenas WKT/WKB:** No debe contener columnas con descripciones `MULTIPOLYGON` u objetos geométricos complejos.
2. **Estructura fija inicial:** Columnas exactas denominadas `id` (entero consecutivo), `longitud` y `latitud` (coordenadas decimales nativas EPSG:4326).
3. **Columnas dinámicas:** Mapeo exitoso e individualizado de coberturas según el nombre del archivo origen de datos (ej. `integridad_mexico` y `reticula_variable`).
...
Nuevamente tienes que regresar a tu `snakefile` para registrar esto y habilitar lo necesario para el reporte. Se trata básicamente de agregar la línea que define el reporte y algunas indicaciones de que productos poner bajo etiquetas descriptivas en la página que se producirá. Un ejemplo de esto se vería así:



```{.python}
{{< include scripts-y-reglas/reglas-paso-5.txt >}}
...
#### Prompt 3 — Agregar simbología ráster y vectorial
...
Seguramente la **IA** te propondrá algo como lo que me dio a mi. Básicamente es lo necesario para abrir el zip que es el proyecto **qgz** y poderlo analizar. Revisa la especificación de las leyendas y los metadatos necesarios para servir los mapas en la Web, utilizando la tecnología que tiene disponible **QGis server**:
...
Para esta capa te sugiero ir a la opción *Layer/Data Source Manager*. Encontré que intentar jalar la capa directamente desde la pestaña browser deja algunos atributos sin definir y la interfaz reporta errores, aunque la cobertura esté bien y sea bien entregada por el servidor. En cambio, por el camino *Data Source Manager* tienes más control y la obtención de los datos fluye de mejor manera. Puedes incluso hacer ahí algo de manejo tipo base de datos **SQL**, que es interesante.

![Data source manager](scripts-y-reglas/img/data-source-manager.png){#fig-data-manager}
...
Comandos clave (seguramente lo que tendrás que poner en el `curl` deberás ajustarlo a tus condiciones):
...
No olvides documentar lo que hiciste. También le puedes pedir ayuda a la **IA** para generar un documento de acompañamiento. No traté aquí nada de la documentación y registro de los metadatos. **Maricruz nos orientará sobre este tema**.
