...
En los pasos siguientes verás versiones progresivas del mismo `snakefile`, que iras construyendo conforme agregues tareas y procesos al flujo de trabajo. Cada vez Que tengas un nuevo `snakefile`, con sus scripts y componentes necesarios, deberás correr el comando `snakemake --cores 1` para verificar que todo está en orden y para obtener los productos involucrados en cada paso.
...
Ya tienes definido el proceso en el `snakefile`. Ve lo que marcaste en las líneas que dicen **script:**, para cada regla. Eso indica que scripts deberás desarrollar para que el flujo de trabajo haga su tarea. A continuación te sugiero los tres scripts que ya están indicados en el `snakefile` que acabas de terminar. Este flujo de trabajo también incluye un reporte en Quarto, que es el último componente de esta primera tarea. Constrúyelos y dales el nombre y ubicación que están registrados en el `snakefile`.
...
Ya tienes todos los ingredientes para hacer una primera corrida del flujo de trabajo. Ejecuta el comando `snakemake --cores 1` y si todo está bien deberás veru un montón de mensajes en pantalla y tendrás los primeros resultados en las carpetas de salida que indicaste en el mismo `snakefile` bajo el encabezado **output**.


## 7. Recolección tabular de las variables
