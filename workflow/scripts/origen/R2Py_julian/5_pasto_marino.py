...
"""
Nombre: 5_pasto_marino.py

Propósito:
    Calcular, para cada píxel de los rasters regionales de referencia, la
    distancia al pasto marino más cercano y serializar el resultado como
...
Resumen del flujo:
    ...
    3. Reproyectar cada raster al CRS del shapefile.
    4. Rasterizar el pasto marino sobre la plantilla regional reproyectada.
    5. Calcular la distancia al pasto marino más cercano para cada píxel.
    6. Reemplazar el valor centinela 999 en regiones sin pasto por 1.5 * max_dist.
...
PASTO_MARINO_SHP = DROPBOX_DIR / "data_crude" / "09_pasto_marino" / "pasto_marino.shp"
REF_GRID_DIR = DROPBOX_DIR / "data" / "06_DunasCost250116_malla_ref_50m"
OUTPUT_PKL = DROPBOX_DIR / "data_features" / "5_pasto_marino.pkl"
...
def rasterize_pasto_on_region(shape, transform, pasto: gpd.GeoDataFrame) -> np.ndarray:
    ...
    Equivalente funcional de:
      pasto_rast <- rasterize(pasto, region_)
    """
    shapes = (
        (geom, 1)
...
def pasto_points_from_raster(pasto_rast: np.ndarray, transform) -> pd.DataFrame:
    """
    Equivalente a:
      pasto_points <- as.data.frame(pasto_rast, xy = TRUE)
...
def nearest_distance_column(points_xy: np.ndarray, pasto_points: pd.DataFrame) -> np.ndarray:
    """
    Distancia euclidiana al pasto más cercano.
...
def process_region(region_path: Path, pasto: gpd.GeoDataFrame) -> pd.DataFrame:
    print(region_path)
    ...
    region_points["pasto"] = PASTO_SENTINEL
    ...
    if np.isfinite(pasto_rast).sum() > 0:
        ...
        # Replica:
        # if (nrow(pasto_points)==1){
        #   pasto_points <- pasto_points[c(1,1),]
        # }
        ...
        region_points["pasto"] = distances

    return region_points


def main() -> None:
    pasto = load_pasto_marino(PASTO_MARINO_SHP)
    c_list = list_reference_grids(REF_GRID_DIR)

    df_list: list[pd.DataFrame] = []

    for region in c_list:
        region_df = process_region(region, pasto)
        df_list.append(region_df)

    full_df = pd.concat(df_list, ignore_index=True)

    max_dist = float(full_df["pasto"].max())
    full_df.loc[full_df["pasto"] == PASTO_SENTINEL, "pasto"] = 1.5 * max_dist
