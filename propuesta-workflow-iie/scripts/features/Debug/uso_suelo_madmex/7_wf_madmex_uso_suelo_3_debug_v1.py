...
"""
Debug: 7_wf_madmex_uso_suelo_3.py

Calcula distancias a clases MADMEX 27, 28 y 29 usando la malla de una tabla
base regional. Esta versión agrega diagnósticos y emula kknn(..., k=1,
distance=2, kernel="rectangular", scale=TRUE) mediante distancias al vecino
más cercano en coordenadas escaladas por la desviación estándar del conjunto
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--madmex-raster", required=True, help="Ruta al raster MADMEX.")
    parser.add_argument(
        "--base-table",
        required=True,
        help="Ruta a tabla base .parquet con regionid, pixid, x, y.",
    )
...
def read_base_table(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"La tabla base no cumple el contrato mínimo. Faltan columnas: {missing}")
...
def transform_points_to_crs(xs: np.ndarray, ys: np.ndarray, src_crs, dst_crs) -> tuple[np.ndarray, np.ndarray]:
    tx, ty = rio_transform(src_crs, dst_crs, xs.tolist(), ys.tolist())
    return np.asarray(tx, dtype=float), np.asarray(ty, dtype=float)


def crop_raster_to_points_extent(src: rasterio.io.DatasetReader, xs: np.ndarray, ys: np.ndarray) -> tuple[np.ndarray, rasterio.Affine]:
...
def raster_points_dataframe(arr: np.ndarray, transform) -> pd.DataFrame:
    ...
    return pd.DataFrame({"x": np.asarray(xs), "y": np.asarray(ys), "value": arr[rows, cols]})
...
def scaled_xy(train_xy: np.ndarray, pred_xy: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    sd = np.std(train_xy, axis=0, ddof=1)
    sd = np.where(np.isfinite(sd) & (sd != 0), sd, 1.0)
    return train_xy / sd, pred_xy / sd, sd


def nearest_distance_column(points_xy: np.ndarray, class_points: pd.DataFrame, distance_mode: str) -> tuple[np.ndarray, tuple[float, float]]:
...
def main() -> None:
    ...
    with rasterio.open(madmex_path) as madmex_src:
        ...
        madmex_arr_raw, madmex_transform = crop_raster_to_points_extent(madmex_src, tx, ty)
    ...
    madmex_points = raster_points_dataframe(madmex_arr, madmex_transform).rename(columns={"value": "layer"})
    ...
    if args.debug_madmex_points_output:
        debug_points = madmex_points.copy()
        save_table(debug_points, Path(args.debug_madmex_points_output))
    ...
    if args.debug_metadata_output:
        ...
        # Agregar conteos por clase en columnas legibles.
        for _, row in class_summary.iterrows():
